...
async def on_ready():
    ...
    while True:
        ...
        # response_game = requests.get('https://api.twitch.tv/helix/streams?user_login=' + Twitch, headers=headers)
        ...
        embed.add_field(name="방송 보러오기", value='https://www.twitch.tv/' + Twitch, inline=True)
        embed.set_footer(text='방송 시작')
        ...
        try:
            ...
            if loads(response_channel.text)['data'][0]['type'] == 'live' and check == False:
                ...
                await channel.send("@everyone\n" + nickname + "님이 방송을 시작했다구! 어서들 보러 와달라구!!", embed=embed)
                # await channel.send("@everyone" + name + "님이 방송을 시작했습니다." + loads(response_channel.text)['data']['name'])
                print("Online")
                check = True
        except:
            print("Offline")
...
#         try:
#             if loads(response_channel.text)['data'][0]['type'] == 'live' and check == False:
#                 await channel.send("@everyone\n" + nickname + "님이 방송을 시작했다구! 어서들 보러 오라구!")
#                 print("Online")
#                 check = True
#         except:
#             print("Offline")
